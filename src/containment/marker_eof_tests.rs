...
/// NOT `nix::unistd::pipe()` (raw POSIX semantics, not CLOEXEC) — because when tests share a
/// process (`cargo test --lib -- --nocapture`), every test in this crate runs concurrently in one
/// process, and a non-CLOEXEC test pipe fd would be inherited by any OTHER concurrently-running
/// test's spawned child, keeping that child a spurious extra "holder" of a pipe this test never
/// intended to share.
...
fn a_retained_supervisor_write_end_is_refused_not_waited_on() {
    ...
    drop(w);
    // NOT `assert_eq!(..., Clear)`: when tests share a process (`cargo test -- --nocapture`) this
...
fn a_quiet_live_holder_blocks_without_spending_cpu() {
    ...
    // concurrently-running tests do on other threads during the same window, when tests share a
    // process (`cargo test -- --nocapture`)).
...
/// tests are running concurrently during the same measurement window, when tests share a process
/// (`cargo test -- --nocapture`). Used only to distinguish "blocked" from "busy-polled" in the
