...
fn occupied_leaf_for_test(dir: &std::path::Path, name: &str) -> std::path::PathBuf {
    ...
    std::fs::create_dir(leaf_path.join("occupant")).expect("stand in for the live tree");
...
fn cgroup_attachment_for_test(leaf_path: &std::path::Path) -> super::Attachment {
    super::Attachment {
        ...
        attached: super::Attached::Cgroup(crate::containment::cgroup::CgroupLeaf::for_test_at(
